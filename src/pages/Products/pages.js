...
<div className="relative mt-12 w-full">
  ...
  <div className="absolute inset-0 flex flex-col items-center justify-center text-center bg-black/40 rounded-2xl">
    ...
    <button className="mt-6 px-10 py-3 bg-gradient-to-r from-[#8B4513] via-[#6D4C41] to-[#3E2723] 
  ...
  <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 px-4 overflow-y-auto">
    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl mx-auto p-6 md:p-10 relative animate-fadeIn border border-[#E0C097]/80">
      ...
      <button
        ...
        className="absolute top-4 right-4 text-gray-600 hover:text-[#6D4C41] text-2xl font-bold"
      >
        ✖
      </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="flex items-center mt-3 text-yellow-500">
            {[...Array(5)].map((_, i) => (
              <FaStar
                key={i}
                className={`h-5 w-5 ${
                  i < selected.rating ? "text-yellow-500" : "text-gray-300"
                }`}
              />
            ))}
          ...
          <div className="mt-6 space-y-3 text-sm sm:text-base text-gray-700">
            <p>
              <span className="font-semibold text-[#3E2723]">Delivery:</span>{" "}
              Available all over Pakistan
            </p>
            <p>
              <span className="font-semibold text-[#3E2723]">Payment:</span>{" "}
              Cash on Delivery
            </p>
          </div>
